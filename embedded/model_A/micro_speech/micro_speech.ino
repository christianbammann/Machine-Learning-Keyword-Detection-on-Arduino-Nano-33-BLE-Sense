...
constexpr bool kEnableInferenceDebug = true;
...
void setup() {
  ...
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    ...
    if (kEnableSetupDebug) {
      Serial.println("setup: bad model schema");
      Serial.flush();
    }
    return;
  }
  ...
  if (micro_op_resolver.AddDepthwiseConv2D() != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "AddDepthwiseConv2D() failed");
    return;
  }
  if (kEnableSetupDebug) {
  ...
  static RecognizeCommands static_recognizer(
      ...
      210,   // Detection threshold on dequantized 0-255 scores.
      2500,  // Suppression time after a command fires.
...
void loop() {
  if (!is_setup_complete) {
    static int32_t last_setup_wait_ms = 0;
    const int32_t now = millis();
    if ((now - last_setup_wait_ms) >= 1000) {
      Serial.println("setup: incomplete");
      Serial.flush();
      digitalWrite(LED_BUILTIN, !digitalRead(LED_BUILTIN));
      last_setup_wait_ms = now;
    }
    return;
  }
