...
namespace {
constexpr int32_t kNonNoiseMargin = 50;
}  // namespace
...
TfLiteStatus RecognizeCommands::ProcessLatestResults(
  ...
  int current_top_index = kSilenceIndex;
  int32_t current_top_score = average_scores[kSilenceIndex];
  for (int i = 0; i < kCategoryCount; ++i) {
  ...
  if (current_top_index == kSilenceIndex) {
  ...
  if ((current_top_score - average_scores[kSilenceIndex]) < kNonNoiseMargin) {
    previous_top_label_ = kCategoryLabels[kSilenceIndex];
    *found_command = kCategoryLabels[kSilenceIndex];
    *score = average_scores[kSilenceIndex];
    *is_new_command = false;
    return kTfLiteOk;
  }
